...
import os
# 切片所在目录
slice_dir_path = '/home/yuhan/桌面/Juliet/C/2_1slice_file'
# 单个标签文件所在目录
label_dir_path = '/home/yuhan/桌面/Juliet/C/2_1label_file'
# 数据集提供的含漏洞信息的xml文件
...
data_file_path = '/home/yuhan/桌面/Juliet/C/data_Juliet.txt'
label_file_path = '/home/yuhan/桌面/Juliet/C/label_Juliet.txt'
info_file_path = '/home/yuhan/桌面/Juliet/C/info_Juliet.txt'
ll_path_file_path = '/home/yuhan/桌面/Juliet/C/ll_path_Juliet.txt'
...
def get_data_and_label(get_data, get_label):
    ...
    for slice_dir in slice_dirs:
        ...
        slcie_names = os.listdir(slice_child_dir_path)
        for slice_name in slcie_names:
            # 打标签
            if get_label == 1:
                ...
                if os.path.exists(single_label_file_path) == 0:
                    command = "./tag/build/IRHandler "+slice_child_dir_path+'/'+slice_name + \
            ...
            if get_data == 1:
                command = ir2vec_path+'/build/bin/ir2vec '+'-fa -vocab ' + ir2vec_path+Vocab_path + \
                    ' -o '+data_file_path + ' -level f '+slice_child_dir_path+'/'+slice_name
                print(command)
                os.system(command)
                command = "echo \"##########\" >>"+data_file_path
                os.system(command)
            # 存信息
            if slice_name.find('_'):
                ...
                info_file.write(slice_dir+'.c '+'function:'+func+'\n')
            # 存路径
            ll_path_file.write(slice_child_dir_path+'/'+slice_name)
    ll_path_file_path.close()
    info_file.close()
    label_file.close()
    data_file.close()
